...
import type { V2AnalysisReport, V2Vulnerability } from '../interfaces/v2-report.interface'

/**
 * Quality metrics for evaluating V2 report reliability
 */
export interface QualityMetrics {
  overallAccuracy: number      // 0-1: Overall report quality score
  languageDetectionScore: number // 0-1: Language detection reliability
  confidenceConsistency: number  // 0-1: How consistent confidence scores are
  misclassificationRisk: number  // 0-1: Risk of misclassified vulnerabilities
  lineAccuracyScore: number     // 0-1: Line number reliability
  patternQuality: number        // 0-1: Pattern matching quality
}
...
export interface QualityGateConfig {
  minOverallAccuracy: number    // Minimum 0.7 (70%) required
  minLanguageConfidence: number // Minimum language detection confidence
  maxMisclassificationRisk: number // Maximum acceptable misclassification risk
  minVulnerabilityCount: number // Minimum vulnerabilities to be useful
}
...
export class QualityGate {
   ...
   */
  shouldDisplayV2(v2Report: V2AnalysisReport | undefined | null): boolean {
    if (!v2Report) {
      return false
    }
   ...
   */
  calculateReportQuality(report: V2AnalysisReport): QualityMetrics {
   ...
   */
  private calculateLanguageDetectionScore(report: V2AnalysisReport): number {
   ...
   */
  private calculateConfidenceConsistency(vulns: V2Vulnerability[]): number {
   ...
   */
  private calculateMisclassificationRisk(vulns: V2Vulnerability[]): number {
    if (vulns.length === 0) {
      return 0
    }
   ...
   */
  private calculateLineAccuracyScore(vulns: V2Vulnerability[]): number {
    const vulnsWithLines = vulns.filter(v =>
      v.affectedCode?.lines && v.affectedCode.lines.length > 0
    )
   ...
   */
  private calculatePatternQuality(vulns: V2Vulnerability[]): number {
    if (vulns.length === 0) {
      return 0
    }
   ...
   */
  getQualityReport(report: V2AnalysisReport): {
