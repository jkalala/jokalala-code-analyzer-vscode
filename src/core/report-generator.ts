...
export interface ReportOptions {
  ...
  maxIssues?: number
  minSeverity?: string
  groupBy?: 'file' | 'severity' | 'category' | 'rule'
  sortBy?: 'severity' | 'file' | 'line' | 'confidence'
  template?: string
  outputPath?: string
}
...
class SarifGenerator {
  ...
  private generateRules(data: ReportData): unknown[] {
  ...
  private generateResults(issues: ReportIssue[], options: ReportOptions): unknown[] {
  ...
  private hashLocation(issue: ReportIssue): string {
...
<body>
  ...
  private groupIssues(issues: ReportIssue[], groupBy: string): Map<string, ReportIssue[]> {
  ...
  private renderGroupedIssues(groups: Map<string, ReportIssue[]>, options: ReportOptions): string {
    let html = ''
  ...
  private renderIssue(issue: ReportIssue, options: ReportOptions): string {
    return `
    <div class="issue">
 ...
 */
class CsvGenerator {
  generate(data: ReportData, _options: ReportOptions): string {
    const headers = [
      'ID',
      'Rule ID',
      'Rule Name',
...
class JunitGenerator {
  generate(data: ReportData, _options: ReportOptions): string {
    ...
    // Group by file
    const byFile = new Map<string, ReportIssue[]>()
    for (const issue of data.issues) {
...
class CodeClimateGenerator {
  ...
  private generateFingerprint(issue: ReportIssue): string {
...
export class ReportGenerator extends EventEmitter {
  private generators: Map<ReportFormat, { generate: (data: ReportData, options: ReportOptions) => string }> = new Map()
