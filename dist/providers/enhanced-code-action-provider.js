...
exports.registerEnhancedCodeActionProvider = registerEnhancedCodeActionProvider;
 ...
 */
const diagnosticVulnerabilityMap = new Map();
/**
...
class EnhancedCodeActionProvider {
     ...
     */
    static registerVulnerability(diagnosticCode, vulnerability, documentUri) {
    ...
    provideCodeActions(document, _range, context, _token) {
        ...
        const diagnostics = context.diagnostics.filter(d => d.source === 'Jokalala Code Analysis' || d.source === 'jokalala');
        for (const diagnostic of diagnostics) {
            ...
            if (vulnData?.vulnerability) {
                ...
                const vulnActions = this.createVulnerabilityActions(document, diagnostic, vuln);
     ...
     */
    createVulnerabilityActions(document, diagnostic, vuln) {
        const actions = [];
        // 1. Primary fix action (if available)
        if (vuln.fix?.secureCode) {
            const primaryFix = this.createApplyFixAction(document, diagnostic, vuln, vuln.fix, true);
            if (primaryFix) {
        ...
        if (vuln.fix?.alternatives && vuln.fix.alternatives.length > 0) {
            for (const alt of vuln.fix.alternatives) {
                const altFix = this.createAlternativeFixAction(document, diagnostic, vuln, alt);
                if (altFix) {
                    actions.push(altFix);
                }
            }
     ...
     */
    createApplyFixAction(document, diagnostic, vuln, fix, isPrimary) {
     ...
     */
    createFixEdit(document, diagnostic, _vuln, fix) {
     ...
     */
    createAlternativeFixAction(document, diagnostic, vuln, alternative) {
     ...
     */
    createCopyFixAction(vuln) {
        const action = new vscode.CodeAction('Copy fix code to clipboard', vscode.CodeActionKind.QuickFix);
        action.command = {
     ...
     */
    createMarkFalsePositiveAction(vuln, diagnostic) {
        const action = new vscode.CodeAction('Mark as false positive', vscode.CodeActionKind.QuickFix);
        action.diagnostics = [diagnostic];
        action.command = {
     ...
     */
    createAcceptVulnerabilityAction(vuln, diagnostic) {
        const action = new vscode.CodeAction('Confirm as real vulnerability', vscode.CodeActionKind.QuickFix);
        action.diagnostics = [diagnostic];
        action.command = {
     ...
     */
    createShowDetailsAction(vuln) {
        const action = new vscode.CodeAction('Show vulnerability details', vscode.CodeActionKind.QuickFix);
        action.command = {
     ...
     */
    createIgnoreRuleAction(vuln) {
        const ruleType = vuln.primaryIssue?.type || 'unknown';
        const action = new vscode.CodeAction(`Ignore all "${ruleType}" findings`, vscode.CodeActionKind.QuickFix);
        action.command = {
     ...
     */
    createBasicActions(_document, diagnostic) {
        const actions = [];
        // Show documentation
        const docAction = new vscode.CodeAction('Show documentation', vscode.CodeActionKind.QuickFix);
...
Object.defineProperty(EnhancedCodeActionProvider, "providedCodeActionKinds", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: [
...
function registerCodeActionCommands(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.copyFix', async (vulnId, fixCode, fixSummary) => {
        await vscode.env.clipboard.writeText(fixCode);
        vscode.window.showInformationMessage(`Fix code copied: ${fixSummary}`);
        // Record feedback
        user_feedback_service_1.userFeedbackService.recordCopiedFix(vulnId, 'unknown', // Would need vuln type
        'unknown', // Would need severity
        0.5, // Would need confidence
        fixSummary);
    }));
    // Mark as false positive
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.markFalsePositive', async (vuln) => {
        // Add to false positive list
        false_positive_detector_1.falsePositiveDetector.suppressVulnerability(vuln.id);
        // Record feedback
        user_feedback_service_1.userFeedbackService.recordRejected(vuln.id, vuln.primaryIssue?.type || 'unknown', vuln.severity, vuln.confidence, vuln.fix?.language, 'User marked as false positive');
        vscode.window.showInformationMessage(`Marked "${vuln.primaryIssue?.title}" as false positive`);
        // Trigger re-analysis to update view
        vscode.commands.executeCommand('jokalala-code-analysis.analyzeFile');
    }));
    // Accept vulnerability
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.acceptVulnerability', (vuln) => {
        // Record feedback
        user_feedback_service_1.userFeedbackService.recordAccepted(vuln.id, vuln.primaryIssue?.type || 'unknown', vuln.severity, vuln.confidence, vuln.fix?.language);
        vscode.window.showInformationMessage(`Confirmed "${vuln.primaryIssue?.title}" as real vulnerability`);
    }));
    // Show vulnerability details
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.showVulnerabilityDetails', (vuln) => {
        const panel = vscode.window.createWebviewPanel('vulnDetails', `Vulnerability: ${vuln.primaryIssue?.title}`, vscode.ViewColumn.Beside, { enableScripts: true });
        panel.webview.html = generateVulnerabilityDetailsHtml(vuln);
    }));
    // Preview fix
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.previewFix', async (_vulnId, fix) => {
        const panel = vscode.window.createWebviewPanel('fixPreview', `Fix Preview: ${fix.quickSummary}`, vscode.ViewColumn.Beside, { enableScripts: true });
        panel.webview.html = generateFixPreviewHtml(fix);
    }));
    // Show alternative
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.showAlternative', async (_vulnId, alternative) => {
        const result = await vscode.window.showInformationMessage(`Alternative: ${alternative.approach}`, { modal: true, detail: `Pros: ${alternative.pros.join(', ')}\nCons: ${alternative.cons.join(', ')}` }, 'Copy Code', 'Cancel');
        if (result === 'Copy Code') {
    ...
    // Ignore rule
    context.subscriptions.push(vscode.commands.registerCommand('jokalala.ignoreRule', async (ruleType) => {
        const config = vscode.workspace.getConfiguration('jokalala');
        const ignoredRules = config.get('ignoredRules', []);
        ...
        vscode.window.showInformationMessage(`Rule "${ruleType}" will be ignored in future analyses`);
        // Trigger re-analysis
        vscode.commands.executeCommand('jokalala-code-analysis.analyzeFile');
    }));
}
 ...
 */
function generateVulnerabilityDetailsHtml(vuln) {
    const confidence = Math.round(vuln.confidence * 100);
    return `<!DOCTYPE html>
<html lang="en">
<head>
 ...
 */
function generateFixPreviewHtml(fix) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
 ...
 */
function registerEnhancedCodeActionProvider(context) {
    const provider = new EnhancedCodeActionProvider();
    const registration = vscode.languages.registerCodeActionsProvider({ scheme: 'file' }, provider, {
...
//# sourceMappingURL=enhanced-code-action-provider.js.map