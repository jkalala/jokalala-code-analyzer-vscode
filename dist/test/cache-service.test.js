...
suite('CacheService Test Suite', () => {
    ...
    setup(() => {
        ...
        cacheService = new cache_service_1.CacheService(mockContext);
    });
    teardown(() => {
    ...
    suite('Basic Cache Operations', () => {
        ...
        test('should clear all values', () => {
            ...
            assert.strictEqual(cacheService.get('key2'), undefined);
        });
        ...
        test('should handle different data types', () => {
            ...
            assert.strictEqual(cacheService.get('string'), 'test');
            assert.strictEqual(cacheService.get('number'), 42);
            assert.strictEqual(cacheService.get('boolean'), true);
            assert.deepStrictEqual(cacheService.get('object'), { foo: 'bar' });
            assert.deepStrictEqual(cacheService.get('array'), [1, 2, 3]);
        });
    });
    ...
    suite('LRU (Least Recently Used) Eviction', () => {
        test('should evict least recently used entry when cache is full', () => {
            ...
            const maxEntries = constants_1.CACHE_DEFAULTS.maxEntries;
            ...
            assert.strictEqual(cacheService.get('key0'), 'value0'); // Should still exist
            assert.strictEqual(cacheService.get('key1'), undefined); // Should be evicted
            assert.strictEqual(cacheService.get('newKey'), 'newValue');
        });
        ...
        test('should update access order on set() for existing keys', () => {
            ...
            assert.strictEqual(cacheService.get('key1'), 'updated');
        });
    });
    ...
    suite('Cache Persistence', () => {
        ...
        test('should restore cache from workspace storage', async () => {
            ...
            const newCacheService = new cache_service_1.CacheService(mockContext);
            await newCacheService.restore();
            assert.strictEqual(newCacheService.get('key1'), 'value1');
            assert.strictEqual(newCacheService.get('key2'), 'value2');
        ...
        test('should not persist expired entries', async () => {
            ...
            const newCacheService = new cache_service_1.CacheService(mockContext);
            await newCacheService.restore();
            assert.strictEqual(newCacheService.get('key1'), undefined);
            assert.strictEqual(newCacheService.get('key2'), 'value2');
            newCacheService.dispose();
        });
        ...
        test('should handle persistence errors gracefully', async () => {
            ...
            const badCacheService = new cache_service_1.CacheService(badContext);
        ...
        test('should throw error when persisting without context', async () => {
            const contextlessCacheService = new cache_service_1.CacheService();
            await assert.rejects(async () => await contextlessCacheService.persist(), /Cannot persist cache: ExtensionContext not provided/);
            contextlessCacheService.dispose();
        });
        test('should throw error when restoring without context', async () => {
            const contextlessCacheService = new cache_service_1.CacheService();
            await assert.rejects(async () => await contextlessCacheService.restore(), /Cannot restore cache: ExtensionContext not provided/);
            contextlessCacheService.dispose();
        });
    });
    suite('Resource Management', () => {
        test('should dispose cleanup timer', () => {
            const service = new cache_service_1.CacheService();
            service.dispose();
