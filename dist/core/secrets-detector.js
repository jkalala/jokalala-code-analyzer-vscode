...
exports.SecretsDetector = exports.SecretType = exports.SecretSeverity = void 0;
exports.createSecretsDetector = createSecretsDetector;
exports.getSecretsDetector = getSecretsDetector;
...
(function (SecretType) {
    ...
    SecretType["WEBHOOK_URL"] = "webhook_url";
    SecretType["OAUTH_SECRET"] = "oauth_secret";
    SecretType["JWT"] = "jwt";
...
class EntropyCalculator {
     ...
     */
    static calculateEntropy(str, charset) {
     ...
     */
    static detectCharset(str) {
        const hasNonHex = /[^0-9a-fA-F]/.test(str);
        const hasNonBase64 = /[^A-Za-z0-9+/=]/.test(str);
        if (!hasNonHex)
            return this.HEX_CHARSET;
        if (!hasNonBase64)
            return this.BASE64_CHARSET;
        return this.ALPHANUMERIC_CHARSET;
    }
    /**
     * Check if a string has suspicious entropy for a secret
     */
    static hasSuspiciousEntropy(str, minEntropy = 3.5, maxEntropy = 6.0) {
        const entropy = this.calculateEntropy(str);
        return entropy >= minEntropy && entropy <= maxEntropy;
    }
}
Object.defineProperty(EntropyCalculator, "BASE64_CHARSET", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/='
});
Object.defineProperty(EntropyCalculator, "HEX_CHARSET", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: '0123456789abcdefABCDEF'
});
Object.defineProperty(EntropyCalculator, "ALPHANUMERIC_CHARSET", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
 ...
 */
class SecretsDetector extends events_1.EventEmitter {
    constructor(options = {}) {
        super();
    ...
    scan(code, filename) {
        ...
        if (this.options.includeProviders?.length) {
            activePatterns = activePatterns.filter(p => this.options.includeProviders.includes(p.provider));
        }
        if (this.options.excludeProviders?.length) {
            activePatterns = activePatterns.filter(p => !this.options.excludeProviders.includes(p.provider));
        }
        // Run each pattern
        ...
        if (this.options.enableEntropyDetection) {
            ...
            for (const secret of entropySecrets) {
                const overlaps = secrets.some(s => s.file === secret.file &&
                    s.line === secret.line &&
    ...
    // Private methods
    findMatches(code, pattern, lines, filename) {
        const matches = [];
    ...
    detectHighEntropyStrings(code, lines, filename) {
        const secrets = [];
        ...
        while ((match = stringPattern.exec(code)) !== null) {
            ...
            if (entropy >= (this.options.minEntropy || 3.5) && entropy <= (this.options.maxEntropy || 6.0)) {
                ...
                const surroundingText = code.substring(Math.max(0, match.index - 50), Math.min(code.length, match.index + match[0].length + 50));
 ...
 */
function createSecretsDetector(options) {
    return new SecretsDetector(options);
}
/**
 * Singleton instance
 */
let secretsDetector = null;
function getSecretsDetector(options) {
    if (!secretsDetector) {
