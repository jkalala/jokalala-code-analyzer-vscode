...
class PatternMatcher {
        ...
        for (const pattern of rule.patterns) {
            if (pattern.type === 'regex') {
                try {
                    ...
                    const negativeRegexes = (pattern.negativePatterns || []).map(np => new RegExp(np, 'gi'));
    ...
    calculateConfidence(matchText, rule) {
    ...
    estimateFalsePositiveLikelihood(matchText, rule) {
...
class OfflineAnalyzer extends events_1.EventEmitter {
    constructor() {
        ...
        Object.defineProperty(this, "patternMatcher", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    analyze(code, language, options = {}) {
        ...
        const applicableRules = this.rules.filter(rule => {
            ...
            if (options.disabledRules?.includes(rule.id))
                return false;
            if (options.enabledCategories && !options.enabledCategories.includes(rule.category))
                return false;
            return true;
        });
        // Sort by priority
