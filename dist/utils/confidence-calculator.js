...
class ConfidenceCalculator {
     ...
     */
    calculateForVulnerability(vuln, context) {
        ...
        const overall = Math.max(0, Math.min(1, (patternMatch * weights.patternMatch) +
    ...
    analyzeContext(vuln, context) {
        ...
        if (vulnType.includes('sql')) {
            ...
            const sqlKeywordsFound = SQL_CONTEXT_KEYWORDS.filter(kw => new RegExp(`\\b${kw}\\b`, 'i').test(allCode)).length;
            ...
            const cmdKeywordsFound = COMMAND_CONTEXT_KEYWORDS.filter(kw => new RegExp(`\\b${kw}\\b`, 'i').test(allCode)).length;
            ...
            const xssKeywordsFound = XSS_CONTEXT_KEYWORDS.filter(kw => new RegExp(`\\b${kw}\\b`, 'i').test(allCode)).length;
     ...
     */
    calculateLanguageConfidence(vuln, context) {
     ...
     */
    verifyLineNumbers(vuln, context) {
     ...
     */
    getHistoricalAccuracy(vulnType) {
     ...
     */
    recalculateConfidence(vuln, context) {
     ...
     */
    getConfidenceDescription(breakdown) {
        const issues = [];
