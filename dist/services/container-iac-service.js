...
exports.ContainerIaCService = void 0;
...
class ContainerIaCService {
     ...
     */
    isFileOfType(filePath, content, type) {
     ...
     */
    analyzeDockerCompose(content, filePath) {
        const issues = [];
        const lines = content.split('\n');
        const patterns = [
            {
     ...
     */
    analyzeKubernetes(content, filePath) {
        const issues = [];
        const lines = content.split('\n');
        const patterns = [
            {
     ...
     */
    analyzeTerraform(content, filePath) {
        const issues = [];
        const lines = content.split('\n');
        const patterns = [
            {
     ...
     */
    analyzeCloudFormation(content, filePath) {
        const issues = [];
        const lines = content.split('\n');
        const patterns = [
            {
    ...
    showIssueDetails(issue) {
        const panel = vscode.window.createWebviewPanel('containerIaCIssue', `${issue.type.toUpperCase()}: ${issue.title}`, vscode.ViewColumn.Beside, { enableScripts: false });
        panel.webview.html = this.getIssueDetailsHtml(issue);
    }
    /**
     * Generate HTML for issue details
     */
    getIssueDetailsHtml(issue) {
        const severityColor = {
            critical: '#dc3545',
            high: '#fd7e14',
            medium: '#ffc107',
...
<body>
     ...
     */
    createResult(issues, scannedFiles, scanType, startTime) {
        return {
            success: true,
            issues,
            scannedFiles,
     ...
     */
    createEmptyResult(scanType, error) {
        return {
            success: false,
            issues: [],
            scannedFiles: 0,
...
exports.ContainerIaCService = ContainerIaCService;
//# sourceMappingURL=container-iac-service.js.map